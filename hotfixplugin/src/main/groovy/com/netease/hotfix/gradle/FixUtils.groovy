...
public class FixUtils {
    ...
    static String processKeyWord = 'vopen'
    static List shouldProcessJarPath = ['com.netease.vopen', 'com\\netease\\vopen']
    static List noProcessClsPath = ['android\\support\\', 'R.class', 'BuildConfig.class']
    static List noProcessClsName = ['com.netease.vopen.fix.Fix',
                                    'com.netease.vopen.hotfix.Utils',
                                    'com.netease.vopen.fix.ReflectUtil',
                                    'com.netease.vopen.fix.SignatureChecker',
                                    'com.netease.vopen.app.BaseApplication',
                                    'com.netease.vopen.app.VopenApp']
    ...
    static init(Project project) {
        ...
        logPath = project.projectDir.absolutePath + "\\log_md5_md5.txt"
    ...
    static boolean shouldProcessJar(String filePath) {
        for (String value : shouldProcessJarPath) {
    ...
    static boolean shouldProcessClass(String filePath, boolean minify) {
        if(!filePath.contains(processKeyWord)) {
            return false
        }
    ...
    static void processClass(String dir, String className) {
        ...
        if (cts == null || cts.length == 0) {
            ...
            constructor.setBody("{\nif(Boolean.FALSE.booleanValue())System.out.println(com.aitsuki.hack.AntilazyLoad.class);\n}")
            c.addConstructor(constructor)
        } else {
            cts[0].insertBeforeBody('if(Boolean.FALSE.booleanValue())System.out.println(com.aitsuki.hack.AntilazyLoad.class);')
        }
        c.writeFile(dir)
        c.detach()
    }
    ...
    static void dx(Project project, String patchDir, String patchName) {
        ...
        if (file.isDirectory() && file.exists()) {
            ...
            if (files != null && files.size() > 0) {
                String buildTools = project.android.buildToolsVersion
                def stdout = new ByteArrayOutputStream()
                project.exec {
                    workingDir patchDir
                    commandLine "$sdkDir\\build-tools\\$buildTools\\dx.bat", '--dex', '--output', "$patchName", '.'
    ...
    static void processJar(File jar, Writer writer, boolean minify) {
        processJar(jar, writer, true, false, null, null, minify)
    }

    static void processJar(File jar, boolean inject, boolean generatePatch, Map<String, String> md5Map, String patchDir, boolean minify) {
        processJar(jar, null, inject, generatePatch, md5Map, patchDir, minify)
    }

    static void processJar(File jar, Writer writer, boolean inject, boolean generatePatch, Map<String, String> md5Map, String patchDir, boolean minify) {
        // 解压jar
        File jarDir = new File(jar.parent, jar.getName().replace('.jar', ''))
        unZipJar(jar, jarDir.absolutePath)
        if (inject) {
        ...
        if (inject) {
            jar.delete()
            zipJar(jarDir, jar.getAbsolutePath())
        }
    ...
    static void processDir(File dir, Writer writer) {
        processDir(dir, writer, true, false, null, null, false)
    }

    static void processDir(File dir, boolean inject, boolean generatePatch, Map<String, String> md5Map, String patchDir, boolean minify) {
        processDir(dir, null, inject, generatePatch, md5Map, patchDir, minify)
    }

    static void processDir(File dir, Writer writer, boolean inject, boolean generatePatch, Map<String, String> md5Map, String patchDir, boolean minify) {
        if (inject) {
