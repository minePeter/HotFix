            ...
            if (minify) {
                dexDebug.inputs.files.files.each { File file ->
                    if(file.isDirectory()) {
                        ...
                        if (file.absolutePath.endsWith('.jar')) {
                            FixUtils.processJar(file, debugOn, generatePatch, md5Map, patchDir, true)
                        }
                    }
                }
                ...
                dexDebug.inputs.files.files.each { File file ->
                    if (file.name.endsWith('.jar') && FixUtils.shouldProcessJar(file.absolutePath)) {
                        ...
                        file.eachFileRecurse FileType.FILES, { File f ->
                            if (f.absolutePath.endsWith('.jar')) {
                                FixUtils.processJar(f, debugOn, generatePatch, md5Map, patchDir, false)
                            }
                        }
//                        FixUtils.processDir(file, debugOn, generatePatch, md5Map, patchDir, false, logWriter)
                    }
                }
            }
            if (generatePatch) {
            ...
            if (minify) {
                dexRelease.inputs.files.files.each { File file ->
                    if(file.isDirectory()) {
                        ...
                        if (file.absolutePath.endsWith('.jar')) {
                            FixUtils.processJar(file, writer, true)
                        }
                    }
                }
            } else {
